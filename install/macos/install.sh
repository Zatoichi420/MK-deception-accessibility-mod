#!/bin/bash
# Install the MK: Deception talking-menu daemon as a per-user launchd agent (macOS).
...
mkdir -p "$INSTALL_DIR"
cp "$SRC_DIR/deception_reader.py" "$SRC_DIR/ra_client.py" \
   "$SRC_DIR/deception_addrs.py" "$SRC_DIR/speak.py" "$INSTALL_DIR/"
...
echo
echo "Installed. Start MK: Deception in RetroArch and you should hear the menus."
echo "  log:  tail -f \"$LOG\""
